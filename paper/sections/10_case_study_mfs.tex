...
\subsection{Ethical Considerations and Coordinated Disclosure}
\label{sec:case_study:ethics}

Auditing production financial infrastructure demands rigorous ethical safeguards. Our research adhered to ACM/IEEE ethical guidelines and standard Coordinated Vulnerability Disclosure (CVD) frameworks~\cite{cavusoglu_cvd, iso_29147_cvd}:
(1)~All analyses were conducted strictly offline on publicly available application binaries downloaded from Google Play; no production services, gateways, or customer accounts were probed or attacked. Zero exploits were constructed.
(2)~All institutional identities and namespaces are pseudonymized, and per-app vulnerability details in Table~\ref{tab:real_mfs_audit} are limited to primary observation categories to mitigate re-identification in a concentrated market. Comprehensive unredacted dossiers—including call graphs, bytecode offsets, and remediation patches—were submitted to the national computer emergency response team (\textbf{BGD e-GOV CIRT}) under case tracking reference \textbf{BGD-CIRT-2025-11-0428}\footnote{Institutional de-anonymization keys are deposited securely for editorial review and are available upon request to the corresponding author.} and the Cyber Security Unit of Bangladesh Bank. All mandatory 90-day embargo periods elapsed prior to paper submission. Multiple institutions confirmed receipt and deployed security patches. The artifact repository is archived at \url{https://github.com/kh0kamoni/Reg2App}.
